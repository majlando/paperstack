...
├─ sections/
│  ├─ 01-introduction.md           # diagrams are ```mermaid blocks inside sections
│  └─ ...
├─ appendices/
│  └─ appendix-a-install.md
├─ figures/
├─ diagrams/
│  └─ rendered/2e06c61f.svg        # generated — content-hashed renders of the
│                                  # mermaid blocks; committed so every group
│                                  # member (and CI) can build without the app
└─ output/report.pdf               # generated, git-ignored
```
...
- DOCX export, CLI (the engine + `scripts/build-report.ts` already are the CLI; packaging it is M6)
