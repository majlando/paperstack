...
**Fourth review round (2026-06-12, four parallel deep reviews + fix pass):** engine build, engine project layer, app store/UI, and infra/Rust each got a dedicated reviewer; 26 verified findings were fixed across two passes the same day. Highlights: Replace All could corrupt text under case-changing Unicode (offsets computed on a lowercased copy); `renameSection` was the one document.yaml writer outside the serialized chain; a second ⚙ click disarmed the metadata form's guards; the emitter escaped Typst markup only at paragraph starts (hard breaks and captions could splice a heading mid-paragraph — a compile error inside containers); math scripts took digit *runs* where TeX takes one digit, so the PDF disagreed with the KaTeX preview; mermaid extraction missed 4-backtick/tilde/blockquoted fences the preview renders; yaml re-emission re-indented hand-written 4-space files; unquoted leading-zero student ids printed wrong on the cover; the sidecar output path could be a committed symlink pointing outside the project; and Unix builds would have refused every write under `output/.build` (`requireLiteralLeadingDot`). The Rust validators are now plain functions with nine unit tests and their own CI job, the webview's sidecar allowlist shrank to typst only, and every store fix is pinned by a GatedPlatform test shown to fail without it.
...
- [x] Deterministic fonts (2026-06-11): builds compile with `--ignore-system-fonts`, so only the fonts embedded in the bundled typst resolve — Libertinus Serif / New Computer Modern / DejaVu Sans Mono, all with full bold/italic variants, no separate font bundle needed. The same commit now renders the same PDF on every OS. A project `fonts/` folder is passed via `--font-path` when present — committed font files are the customization path that stays deterministic for the whole group. Cambria/Consolas left the template stacks (they were unreachable fallbacks); the sidecar allowlist validates the new invocation shapes
- [x] Vendor the template into the project + offer updates (2026-06-11): vendoring shipped earlier (first build writes the Git-tracked `paperstack-template.typ`); now the engine keeps CR-insensitive hashes of every stock template ever shipped (`templateStatus`), and the app shows an update **offer** when a project's copy is unmodified-but-outdated stock — user-customized templates are never touched or even prompted about. Declining mutes the offer for the session. A pin test fails whenever SEA_TEMPLATE changes until the old hash is registered
...
| Same commit renders different PDFs per group member's OS (system fonts) | **Cleared (2026-06-11)** — `--ignore-system-fonts` + embedded-only default stack; project `fonts/` folder for deterministic customization |
