...
- `pnpm test` and `pnpm typecheck` — run both before every commit; CI runs these plus the desktop Vite build (`pnpm --filter @paperstack/desktop build`). The PDF integration test needs `bin/` (populate via `pwsh ./scripts/fetch-binaries.ps1`) and auto-skips without it.
