...
- `pnpm smoke` — scripted end-to-end smoke test: scaffolds a scratch project, launches the app with `VITE_SMOKE_SCRIPT=1`, drives open → edit → save → TODO confirm → export, and asserts the app-written `output/smoke-result.json`. Local only (sidecars + desktop session + free port 1420); run it after changes to the store, platform, or build pipeline.
